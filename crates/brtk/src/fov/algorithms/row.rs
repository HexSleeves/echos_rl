...
impl Row {
    ...
    pub fn calc_starting_slope(&mut self, tile: (i32, i32)) { self.start_slope = Self::slope(tile); }

    pub fn calc_ending_slope(&mut self, tile: (i32, i32)) { self.end_slope = Self::slope(tile); }

    const fn slope(tile: (i32, i32)) -> Slope { Slope::new(2 * tile.1 - 1, 2 * tile.0) }
    ...
    pub fn tiles(&self) -> RowIter {
        ...
        RowIter { depth: self.depth, current_col: start_col, max_col: end_col }
    }
