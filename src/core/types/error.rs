...
mod tests {
    ...
    fn test_error_categories() {
        assert_eq!(GameError::InvalidTarget.category(), "Action");
        assert_eq!(GameError::EntityNotFound(Entity::from_raw(1)).category(), "Entity");
        assert_eq!(
            GameError::MovementBlocked {
    ...
    fn test_error_properties() {
        assert!(GameError::Retry(Box::new(MockAction)).is_retryable());
        assert!(GameError::EntityNotFound(Entity::from_raw(1)).is_fatal());
        assert!(GameError::InvalidTarget.is_warning());
    }

    #[test]
    fn test_user_messages() {
        let error = GameError::OutOfRange { distance: 10.0, max_range: 5.0 };
        assert_eq!(error.user_message(), "Out of range: 10.0 > 5.0");

        let error = GameError::InsufficientMana { required: 50, available: 30 };
        assert_eq!(error.user_message(), "Not enough mana: need 50, have 30");
    }

    #[test]
    fn test_multiple_errors() {
        let errors = vec![GameError::InvalidTarget, GameError::InsufficientResources];
        let combined = GameError::multiple(errors);

        match combined {
            GameError::Multiple(errs) => assert_eq!(errs.len(), 2),
